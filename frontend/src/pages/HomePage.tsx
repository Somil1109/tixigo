...
export function HomePage() {
  const today = new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short" }).format(new Date());
  return <main>
    <section className="hero">
      <div className="hero-glow" />
      <div className="hero-copy"><span className="eyebrow">NOW PLAYING</span><h1>Stories worth<br /><em>showing up for.</em></h1><p>Find your next favourite film, choose your seats, and let the magic begin.</p><div className="hero-actions"><Link to="/movies" className="button primary"><Play size={16} fill="currentColor" /> Book tickets</Link><Link to="/movies" className="button ghost">Explore movies <ArrowRight size={16} /></Link></div></div>
      <div className="hero-poster"><div className="poster-sun" /><span>THE LAST<br />HORIZON</span><small>AN ARJUN MEHRA FILM</small></div>
    </section>
    <section className="quick-search"><div><MapPin size={18} /><span>Choose city</span><strong>Mumbai</strong></div><div><CalendarDays size={18} /><span>Date</span><strong>Today, {today}</strong></div><Link to="/movies?city=Mumbai">Find movies <ArrowRight size={17} /></Link></section>
